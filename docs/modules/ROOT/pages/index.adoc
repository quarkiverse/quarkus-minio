...
Containers are started using Testcontainers and support reusable instances. If you add the property `testcontainers.reuse.enable=true` in your
Testcontainers configuration file, then the container will not be stopped after each run, and can be reused.
